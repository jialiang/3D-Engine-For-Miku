...
float averageBlockerDistance(vec2 uv, float receiverDistance, vec2 texelSize) {
...
float shadowLightFactor(vec4 shadowPosition, vec2 texelSize) {
    ...
    float blockerDistance = averageBlockerDistance(shadowUv.xy, receiverDistance, texelSize);
    ...
    float inShadowPercentage = shadowedFraction(shadowUv.xy, receiverDistance, filterRadiusUv);
